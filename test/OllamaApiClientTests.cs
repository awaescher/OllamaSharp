	...
	public class OllamaApiClientTests
	{
		...
		public class ShowMethod : OllamaApiClientTests
		{
			...
			public async Task Returns_Deserialized_Models()
			{
				...
				var info = await _client.ShowModelAsync("codellama:latest");
		...
		public class GenerateEmbeddingsMethod : OllamaApiClientTests
		{
			...
			public async Task Returns_Deserialized_Models()
			{
				...
				var info = await _client.GenerateEmbeddingsAsync(new EmbeddingsRequest { Model = "", Prompt = "" });

				info.Embeddings.Should().HaveCount(5);
				info.Embeddings.First().Should().BeApproximately(0.567, precision: 0.01);
			}
		}
	}
}